...
class DBHandler_Anko(context : Context) : SQLiteOpenHelper(context, DB_Name, null, DB_Version){
    ...
    companion object {
        private var instance : DBHandler_Anko? = null
        val DB_Version = 1
        val DB_Name = "user.db"

        @Synchronized
        fun getInstance(context : Context) : DBHandler_Anko {
            if (instance == null)
                instance = DBHandler_Anko(context.applicationContext)
            return instance as DBHandler_Anko
        }
    }
